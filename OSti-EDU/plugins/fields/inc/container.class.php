...
class PluginFieldsContainer extends CommonDBTM {
   ...
   static function getEntries($type = 'tab', $full = false) {
      ...
      $sql_type = "1=1";
      if ($type !== "all") {
         $sql_type = "(`type` = '$type' OR `type` = 'remtab')";
      }
      ...
      foreach ($found as $item) {
         ...
         if($item["type"]=="remtab"){
            echo "<style>a[title=".$item["label"]."]{display:none;}</style>";
            //header("location: /OSti/ajax/common.tabs.php?_target=/OSti/front/ticket.form.php&_itemtype=Ticket&_glpi_tab=Ticket$1&id=3&");
         }
         if($item["label"]=="Chamado"){
            echo "<style>form[name='form_ticket']{display:none;}</style>";
         }
   ...
   static function displayTabContentForItem(CommonGLPI $item, $tabnum = 1, $withtemplate = 0) {
      ...
      $container = new self;
      $found_c   = $container->find("(`type` = 'tab' OR `type` = 'remtab')  AND `name` = '$tabnum' AND is_active = 1");
