...
function hack(){
    var s = window.location.search.substring(6);
    s=s.replace(new RegExp("%22", 'g'),"\"");
    console.log(s);
    if(s!=""){
    var indb = JSON.parse(s);
        if(indb.matric.substring(0,1)=="0"){
            indb.matric=indb.matric.substring(1);
        }
        if(indb.matric.substring(0,1)=="0"){
            indb.matric=indb.matric.substring(1);
        }
    console.log(indb)
        var m = document.getElementById("rgf");
        m.value = indb.matric;
        var p = document.getElementById("pass");
        p.value = indb.cpf;
        checkRGF();
        console.log("hacked" + JSON.stringify(indb));
        finalizar();
    }
}
...
function buscRGF(obj, comando, parametros, objetos){
    if(document.getElementById("rgf").value != ""){
        ...
        req.open("POST", "<%=request.getContextPath()%>/SMARrh.jsp", false);
...
<body onLoad="inicializa();hack();">
    <table cellspacing="0" cellpadding="0" align="center" width="100%" height="100%" style="display: none">
        <tr>
                                ...
                                <td width="100%">
                                    <table align="center">
                                        ...
                                        <tr>
                                          ...
                                          <td width="10"></td>
                                          <td><input type="text" id="usuario" name="usuario" onchange="finalizar();" class="textfield"></td>
                                        </tr>
                                        <tr>
                                          <td align="right"><font class="textoPreto">Senha :</font></td>
                                          <td width="10"></td>
                                          <td><input id="pass" type="password" name="senha" class="textfield"></td>
                                        </tr>
                                    </table>
                                </td>
                                <td>
