loadMainScripter('libs/gmail-send/index.js');
...
function lst_load() {
    load(() => {
        getlist(() => {
            var ptable = indb.addons['holerite']
...
function getlist(callback) {
    p2p_send({ status: "ping" }, cfg.dbcon + "addons/lst_holerite", function(result) {
        if (result.status == "pong") {
            indb.addons['holerite'] = result.data;
            callback();
        } else {
...
function openFormulario() {
    ...
    mb.contentDocument.write("<link rel = \"stylesheet\" type=\"text/css\" href=\"../../../css/main.css\">" +
        ...
        "<button onclick= parent.form(document);>Enviar</button></td></tr></table><center></div>" +
        "");
...
function form(doc) {
    ...
    if (!!mess &&
        ...
        p2p_send({ name: nam, matric: mtc, mess: mess, cpf: cpf }, cfg.dbcon + "addons/mail_holerite", function(result) {
